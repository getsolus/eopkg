    ...
    def __check_filesdb(self, force_rebuild=False):
        ...
        if not os.path.exists(files_db):
            ...
            try:
                ...
                if verbose:
                    ctx.ui.info(msg)
                ctx.ui.debug("Are we getting needs_rebuild here? 5")
                needs_rebuild = True
            except:
                can_write = False
                if verbose:
        ...
        # path exists
            ...
            if not "dbm" in db_type:
                db_type = "unknown"
            if os.access(files_db, os.W_OK) and not ctx.get_option("read_only_filesdb"):
                can_write = True
                # we need dbm.gnu here for success
                if db_type != "dbm.gnu":
                    if verbose:
                        ctx.ui.info(
                            _("FilesDB %s is writable, but is of wrong type '%s'.")
                            % (files_db, db_type)
                        )
                    flag = "n"
                    ctx.ui.debug("Are we getting needs_rebuild here? 4")
                    needs_rebuild = True
                else:
                    flag = "w"
                ...
                if db_type != "dbm.gnu":
                    if verbose:
                        ctx.ui.info(
                            _("FilesDB %s is readable, but is of wrong type '%s'.")
                            % (files_db, db_type)
                        )
                    please_rebuild_manually = True
                else:
                    if verbose:
                        ctx.ui.info(
                            _(
                                "Cannot open FilesDB %s for writing. Opening it read-only."
                            )
                            % files_db
                        )
                    flag = "r"
                ...
                if verbose:
                    ctx.ui.warning(
                        _("FilesDB %s of type '%s' exists, but we cannot access it.")
                        % (files_db, db_type)
                    )
                please_rebuild_manually = True
        ...
        if flag is not None:
            ...
            try:
                ...
                if verbose:
                    ctx.ui.debug(
                        "shelve.open(files_db=%s, flag=%s, protocol=%s) failed."
                ...
                if can_write:
                    if verbose:
                        ctx.ui.info(msg)
                    ctx.ui.debug("Are we getting needs_rebuild here? 3")
                    needs_rebuild = True
                else:
                    if verbose:
                        ctx.ui.warning(msg)
            ...
            if valid_shelve and file_exists:
                try:
                    ...
                    if can_write:
                        if verbose:
                            ctx.ui.info(msg)
                        ctx.ui.debug("Are we getting needs_rebuild here? 2")
                        needs_rebuild = True
                    else:
                        if verbose:
                            ctx.ui.warning(msg)
                ...
                if version is not None:
                    if version != FILESDB_FORMAT_VERSION:
                        msg = _("FilesDB is version %s, need version %s.") % (
                            version,
                        ...
                        if can_write:
                            if verbose:
                                ctx.ui.info(msg)
                            ctx.ui.debug("Are we getting needs_rebuild here? 1")
                            needs_rebuild = True
                        else:
                            if verbose:
                                ctx.ui.warning(msg)
        ...
        if please_rebuild_manually:
            ctx.ui.warning(
                _("FilesDB is invalid. Please rebuild it with 'sudo eopkg.py3 -y rdb'")
            )

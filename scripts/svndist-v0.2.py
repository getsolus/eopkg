...
if __name__ == "__main__":
    ...
    if opts.rev == "HEAD":
        ...
        if opts.date == True:
            ...
            os.rename(
                "pisi-%s.tar.bz2" % ver,
                "pisi-%s.%s.%s-%s.tar.bz2" % (time.year, time.month, time.day, ver),
            )
        ...
        run(
            "svn export -r %s %s %s/pisi-r%s"
            % (opts.rev, opts.pisiSVNpath, opts.svndist, opts.rev)
        )
        os.chdir(opts.svndist)
        ...
        if opts.date == True:
            time = datetime.datetime.now()
            os.rename(
                "pisi-r%s.tar.bz2" % opts.rev,
                "pisi-r%s-%s.%s.%s.tar.bz2"
                % (opts.rev, time.year, time.month, time.day),
            )

...
def update(GPG):
    ...
    cmd = GPG_CMD + " --quiet --batch --keyring %s --export | %s --import" % (
        ARCHIVE_KEYRING,
        GPG,
    )
    print("cmd: " + cmd)
    ...
    if os.access(REMOVED_KEYS, os.R_OK):
        ...
        cmd = "%s --keyring %s --with-colons --list-keys | grep ^pub | cut -d: -f5" % (
            GPG_CMD,
            REMOVED_KEYS,
        )
        ...
        for key in keys:
            cmd = (
                "%s --list-keys --with-colons | grep ^pub | cut -d: -f5 | grep -q %s"
                % (GPG, key)
            )
            pipe = subprocess.Popen(
                cmd.split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE
...
def printUsage():
    ...
    print()
    print(
        "  pisi-key add <file>          - add the key contained in <file> ('-' for stdin)"
    )
    ...
    print(
        "  pisi-key adv                 - pass advanced options to gpg (download key)"
    )
    print()
    print(
...
if __name__ == "__main__":
    ...
    if sys.argv[argc] == "--keyring":
        ...
        if not os.access(keyring, F_OK):
            print(
                "Error: The specified keyring %s is missing or not readable" % keyring
            )
            sys.exit(1)
